...
```bash
docker run -d --name mariadb --network test_network --env-file .env my_mariadb
```
...
docker run -d --name wordpress_test --network test_network --env-file .env my_wordpress
```

Go inside to check. Let's check if your WordPress files are in place:
```bash
docker exec -it wordpress_test ls -la /var/www/html
```
...
```bash
docker exec -it wordpress_test wp db check --allow-root
```
...
```bash
docker exec -it wordpress_test wp db tables --allow-root
```
...
```bash
docker exec -it wordpress_test ss -tuln
```
...
Solution:
```bash
docker exec -it wordpress_test grep "listen =" /etc/php/8.1/fpm/pool.d/www.conf
```
...
```bash
docker exec -it wordpress_test apt update
docker exec -it wordpress_test apt install -y iproute2
docker exec -it wordpress_test ss -tuln
```
...
```bash
docker exec -it wordpress_test wp user create test_user test@example.com --role=author --user_pass=password123 --allow-root
```

And then check through MariaDB if it appeared there:

```bash
docker exec -it mariadb mariadb -u mari_unix -p -e "USE inception_db; SELECT user_login FROM wp_users;"
```

Remove the old container: 
```bash
docker rm wordpress_test
```
